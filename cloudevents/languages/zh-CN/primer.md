...
一个[事件](spec.md#event)包含了[事件发生](spec.md#occur)的上下文和相关数据。
...
1. [基本规范](spec.md) 定义了一个抽象信息模型，
   该模型由属性（键值对）和构成 CloudEvent 的相关规则组成。此规范包含了*核心属性*的定义。有些核心属性是必须出现在所有的 CloudEvents 中，有些则是可选的。
...
本节介绍了与CloudEvent核心属性相关的其它背景和设计要点。
