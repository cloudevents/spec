...
- When review/testing is completed, create a PR (for the "main" branch") that:
  ...
  - Update [RELEASES.md](RELEASES.md) to mention the new release, and
    reference the yet-to-be-created release branch.
  - Update the appropriate `*/RELEASE_NOTES.md` file with the changes
    for the release. Thelist can be generated via:
    ...
    to generate the list w/o actually creating the release.

- Merge the PR.
  - Note that the link checker should fail since any references to the new
    release branch will not be valid yet. This is expected.
...
- Create a new branch with the same name as the new release version string.
  ...
  - Note that we do not use Github's "Releases" because their Releases are
    associated with a "tag" and we want our Releases to point to the `HEAD`
    of the corresponding release branch. This allows us to make very minor
    changes (e.g. a PR to fix syntax errors) without needing to create a
    new release, or to change the tag associated with a Github release.
    Ensure that any changes are made in both the "release branch" and "main".
